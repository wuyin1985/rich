    ...
    pub fn traverse_aabb_recursive(
        ...
        match nodes[node_index] {
            BVHNode::Node {
                ...
                if is_aabb_intersect(&aabb, &child_l_aabb) {
                    BVHNode::traverse_aabb_recursive(nodes, child_l_index, aabb, indices);
                }

                if is_aabb_intersect(&aabb, &child_r_aabb) {
                    BVHNode::traverse_aabb_recursive(nodes, child_r_index, aabb, indices);
                }
            }
            BVHNode::Leaf { shape_index, .. } => {
                indices.push(shape_index);
...
impl BVH {
    ...
    /// Traverses the [`BVH`].
    /// Returns a subset of `shapes`, in which the [`AABB`]s of the elements were hit by `AABB`.
    pub fn traverse_aabb<'a, Shape: Bounded>(&'a self, aabb: &AABB, shapes: &'a [Shape]) -> Vec<&Shape> {
        let mut indices = Vec::new();
        BVHNode::traverse_aabb_recursive(&self.nodes, 0, aabb, &mut indices);
        indices
            .iter()
            .map(|index| &shapes[*index])
            .collect::<Vec<_>>()
